    ...
    class SingleDeserializer : Deserializer<float> {
        public override float Read(Reader reader, int tag) {
            ...
            switch (tag) {
                case TagInteger:
                case TagLong:
                    return ValueReader.ReadLongAsSingle(stream);
                case TagDouble:
                ...
                case TagEmpty:
                    return 0;
                case TagUTF8Char:
                    return Converter<float>.Instance.Convert(ValueReader.ReadUTF8Char(stream));
                case TagString:
                    return Converter<float>.Instance.Convert(ReferenceReader.ReadString(reader));
                default:
