...
namespace Hprose.RPC {
    public class TcpSocketHandler : IHandler<Socket> {
        ...
        public TcpSocketHandler(Service service) {
            Service = service;
        }
        ...
        private static async Task<byte[]> ReadAsync(Socket socket, byte[] bytes, int offset, int length) {
            ...
            while (length > 0) {
                ...
                offset += size;
                length -= size;
                buffer = new ArraySegment<byte>(bytes, offset, length);
            }
            return bytes;
        }
